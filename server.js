...
app.get("/api/current-playlists/:timeSlot", function (req, res) {
  ...
  var playlist = currentPlaylists.playlists.filter(
...
app.get("/api/previous-playlists", function (req, res) {
  if (Object.keys(req.query).length === 0) {
    ...
    if (req.query.title != undefined) {
      res.json(
        previousPlaylists.playlists.filter((playlist) =>
...
app.get("/api/songs", function (req, res) {
  if (Object.keys(req.query).length === 0) {
    ...
    if (req.query.title != undefined) {
      ...
      res.json(
        songs.songs.filter((song) => song.songId === Number(req.query.id))
      );
    }
  }
});
...
app.post("/api/current-playlists", function (req, res) {
  var foundPlaylist = currentPlaylists.playlists.find(
    (playlist) => playlist.playlistId == req.body.playlistId
  );
  var selectedSongs = songs.songs.filter((song) =>
